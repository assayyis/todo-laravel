<x-app-layout>
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('todos.store') }}">
            ...
            <textarea
                name="item"
                placeholder="{{ __('What to do today?') }}"
            ...
            @foreach ($todos as $todo)
                <div class="p-6 flex space-x-2">
                    ...
                    <div class="flex-1">
                                ...
                                <x-dropdown>
                                    ...
                                    <x-slot name="content">
                                        ...
                                        <form method="POST" action="{{ route('todos.destroy', $todo) }}">
                                            @csrf
                                            @method('delete')
                                            <x-dropdown-link :href="route('todos.destroy', $todo)" onclick="event.preventDefault(); this.closest('form').submit();">
                                                {{ __('Delete') }}
                                            </x-dropdown-link>
                                        </form>
                                    </x-slot>
                        ...
                        <p class="mt-4 text-lg text-gray-900">{{ $todo->item }}</p>
                    </div>
                </div>
            @endforeach
        </div>
